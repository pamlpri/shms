				...
				<div class="card-body">
					...
					<form id="schlForm" enctype="multipart/form-data" method="post">
									...
									<div class="col-lg-12 row">
										<div class="col-md-4">
											<select class="form-select float-right" name="schlship_no">
												...
												<option value="6"
													${schl.schlship_nm eq '외국어 '?'selected':'' }>외국어</option>
												<option value="7"
													${schl.schlship_nm eq '기사 '?'selected':'' }>기사</option>
												<option value="8"
													${schl.schlship_nm eq '산업기사 '?'selected':'' }>산업기사</option>
												<option value="5"
													${schl.schlship_nm eq '국가유공자 '?'selected':'' }>국가유공자</option>
											</select>
										</div>
									</div>
								</td>
										...
										<td colspan="3" class="text-left"><c:forEach
												...
												<div class="ml-2 fileArea">
													<c:url value="/lms/schlCampusDownload.do" var="downloadURL">
														<c:param name="atch_file_no"
															value="${attach.atch_file_no }" />
						...
						<c:choose>
							<c:when test="${not empty schl.req_no }">
								<div class="form-inline fileArea row">
									<p class="fileBox col-lg-6">
										<input class="form-control" type="file" name="common_files"><span
						...
						<div class="text-center mt-3">
							<a href="${cPath }/lms/scholarshipCampus.do"
								class="btn btn-light-secondary">취소</a>
		...
		<div class="modal-dialog modal-dialog-scrollable" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="deleteForm" action="${cPath }/lms/deleteSchl.do"
						method="post">
						<input type="hidden" name="req_no" value="${schl.req_no }" />
						<c:if test="${not empty  schl}">
							<input type="hidden" name="atch_file_no"
								value="${schl.atch_file_no  }" />
							<c:forEach items="${schl.attachList }" var="attach">
								<div>${attach.atch_file_seq }</div>
								<input type="hidden" name="delAttNos"
									value="${attach.atch_file_seq }" />

							</c:forEach>
						</c:if>
						<p>
							삭제한 신청글은 복원이 불가합니다.<br /> 삭제하시겠습니까?
						</p>
					</form>
				</div>
...
// 	$(".modal").removeClass("show").css("display", "none");
	...
	$("#saveBtn").on("click", function() {
		...
		if (schlship_no == "" && files.length == 0) {
			$(".modal-title").text("장학금 미선택/첨부파일 미 제출");
			$(".modal-body").text("장학금 종류와 첨부파일을 제출해주세요.");
			$("body").addClass("modal-open").css("padding-right", "17px");
			$(".modal-backdrop").addClass("show").css("display", "block");
			$(".modal").addClass("show").css("display", "block");
			$("#realDelBtn").css("display", "none");
			frag[2] = false;
		} else if (schlship_no == "" && files.length != 0) {
			$(".modal-title").text("장학금 미 선택");
			$(".modal-body").text("장학금 종류를 선택해주세요.");
			$("body").addClass("modal-open").css("padding-right", "17px");
			$(".modal-backdrop").addClass("show").css("display", "block");
			$(".modal").addClass("show").css("display", "block");
			$("#realDelBtn").css("display", "none");
			...
			$(".modal-title").text("첨부파일 미제출");
			$(".modal-body").text("첨부파일은 필수입니다.");
			$("body").addClass("modal-open").css("padding-right", "17px");
			$(".modal-backdrop").addClass("show").css("display", "block");
			$(".modal").addClass("show").css("display", "block");
			$("#realDelBtn").css("display", "none");
