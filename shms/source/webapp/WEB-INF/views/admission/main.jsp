<%--
* [[개정이력(Modification Information)]]
* 수정일                 수정자      수정내용
* ----------  ---------  -----------------
* 2021. 5. 20.      박초원        최초작성
* Copyright (c) ${year} by DDIT All right reserved
 --%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<div class="count-down-area pt-90 pb-60 section-bg"
	style="padding-top: 3%; padding-bottom: 0%"
	data-background="${cPath }/resources/admission/assets/img/gallery/section_bg01.png">
	<div class="container" id="registerArea">
		<div class="row justify-content-center">
			<div class="col-lg-12 col-md-12">
				<div class="count-down-wrapper">
					<div class="row justify-content-between">
						<div class="col-lg-3 col-md-6 col-sm-6">
							<!-- Counter Up -->
							<a href="register.html" class="single-counter text-center"> <svg
									xmlns="http://www.w3.org/2000/svg"
									xmlns:xlink="http://www.w3.org/1999/xlink" width="72px"
									height="71px">
                                            <image x="0px" y="0px"
										width="72px" height="71px"
										xlink:href="data:img/png;base64," />
                                       </svg>
								<p class="color-green">원서접수</p>
							</a>
						</div>
						<div class="col-lg-3 col-md-6 col-sm-6">
							<!-- Counter Up -->
							<a href="registerCheck.html" class="single-counter text-center">
								<svg xmlns="http://www.w3.org/2000/svg"
									xmlns:xlink="http://www.w3.org/1999/xlink" width="72px"
									height="71px">
                                             <image x="0px" y="0px"
										width="72px" height="71px"
										xlink:href="data:img/png;base64," />
                                        </svg>
								<p class="color-blue">접수확인</p>
							</a>
						</div>
						<div class="col-lg-3 col-md-6 col-sm-6">
							<!-- Counter Up -->
							<a href="registerSearch.html" class="single-counter text-center">
								<svg xmlns="http://www.w3.org/2000/svg"
									xmlns:xlink="http://www.w3.org/1999/xlink" width="72px"
									height="71px">
                                             <image x="0px" y="0px"
										width="72px" height="71px"
										xlink:href="data:img/png;base64," />
                                        </svg>
								<p class="color-green">합격자조회</p>
							</a>
						</div>
						<div class="col-lg-3 col-md-6 col-sm-6">
							<!-- Counter Up -->
							<a href="registerPay.html" class="single-counter text-center">
								<svg xmlns="http://www.w3.org/2000/svg"
									xmlns:xlink="http://www.w3.org/1999/xlink" width="72px"
									height="71px">
                                            <image x="0px" y="0px"
										width="72px" height="71px"
										xlink:href="data:img/png;base64," />
                                        </svg>
								<p class="color-red">
									등록금 고지서<br />출력 및 납부확인
								</p>
							</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<!--? slider Area Start-->
<div class="slider-area ">
	<div class="slider-active">
		<!-- Single Slider -->
		<div class="single-slider slider-height d-flex align-items-center">
			<div class="container">
				<div class="row align-items-center">
					<div class="col-xl-6 col-lg-7 col-md-8">
						<div class="hero__caption">
							<span data-animation="fadeInLeft" data-delay=".2s">전국 수석
								합격자 연속배출(최근 5년)</span>
							<h1 data-animation="fadeInLeft" data-delay=".4s">
								취업 고민?<br />높은 국가시험<br />합격률로 해결!
							</h1>
							<!-- Hero-btn -->
							<div class="hero__btn">
								<a href="register.html" class="btn hero-btn"
									data-animation="fadeInLeft" data-delay=".8s">원서접수</a>
							</div>
						</div>
					</div>
					<div class="col-xl-6 col-lg-5">
						<div class="hero-man d-none d-lg-block f-right"
							data-animation="jello" data-delay=".4s">
							<img src="${cPath }/resources/admission/assets/img/hero/heroman.png" alt="">
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<!-- slider Area End-->
<!--? Categories Area Start -->
<div class="categories-area section-padding30">
	<div class="container" id="registerMenu">
		<div class="row justify-content-sm-center">
			<div class="cl-xl-7 col-lg-8 col-md-10">
				<!-- Section Tittle -->
				<div class="section-tittle text-center mb-70">
					<span>신편입생 입학</span>
					<h2>입학상담안내</h2>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-lg-4 col-md-6 col-sm-6">
				<div class="single-cat mb-50">
					<div class="cat-icon">
						<span class="flaticon-education"></span>
					</div>
					<div class="cat-cap">
						<h5>
							<a href="helperPresentation.html">입시설명회</a>
						</h5>
						<p>인재대학교 입시현황 및 합격으로 가는 면접팁을 알려드립니다. 학생들과 선생님들의 많은 관심 바랍니다.</p>
						<a href="helperPresentation.html" class="read-more1">Read More
							></a>
					</div>
				</div>
			</div>
			<div class="col-lg-4 col-md-6 col-sm-6">
				<div class="single-cat mb-50">
					<div class="cat-icon">
						<span class="flaticon-tools-and-utensils"></span>
					</div>
					<div class="cat-cap">
						<h5>
							<a href="helperCalendar.html">입시캘린더</a>
						</h5>
						<p>인재대학교의 신편입생 모집 현황을 한 번에 확인할 수 있습니다. 인재대학교의 입시일정을 확인해보세요!</p>
						<a href="helperCalendar.html" class="read-more1">Read More ></a>
					</div>
				</div>
			</div>
			<div class="col-lg-4 col-md-6 col-sm-6">
				<div class="single-cat mb-50">
					<div class="cat-icon">
						<span class="flaticon-business"></span>
					</div>
					<div class="cat-cap">
						<h5>
							<a href="helperGradeCalculation.html">합격예측</a>
						</h5>
						<p>Sorem hpsum folor sixdsft amhtget, consectetur adipiscing
							eliht, sed do eiusmod tempor incidi.</p>
						<a href="helperGradeCalculation.html" class="read-more1">Read
							More ></a>
					</div>
				</div>
			</div>
		</div>
		<!-- Section Button -->
		<div class="row">
			<div class="col-lg-12">
				<div class="browse-btn2 text-center mt-50">
					<a href="helperNotice.html" class="btn">입학도우미</a>
				</div>
			</div>
		</div>
	</div>
</div>
<!-- Categories Area End -->

<!--? About Law Start-->
<div class="about-area section-padding2">
	<div class="container">
		<div class="row">
			<div class="col-lg-6 col-md-12">
				<div class="about-caption mb-50">
					<!-- Section Tittle -->
					<div class="section-tittle mb-35">
						<span>More About Our University</span>
						<h2>Want to know injae</h2>
					</div>
					<p>인재대학교는 성실·인화·창조의 정신으로 심오한 학문의 이론과 응용을 연구하고 가르치며, 이를 통해 인류사회를
						선도할 인재를 양성하고 사회혁신을 이끌 산학협력을 발전시켜, 국가 및 인류의 발전과 번영에 기여하는 것을 목적으로
						합니다.</p>
					<ul>
						<li><span class="flaticon-business"></span> 창의적인 인재</li>
						<li><span class="flaticon-communications-1"></span> 박애적인 인재</li>
						<li><span class="flaticon-graduated"></span> 전문적인 인재</li>
						<li><span class="flaticon-tools-and-utensils"></span> 탐구하는 인재</li>
					</ul>
					<a href="../main/message.html" class="btn">More About Us</a>
				</div>
			</div>
			<div class="col-lg-6 col-md-12">
				<!-- about-img -->
				<div class="about-img ">
					<div class="about-font-img d-none d-lg-block">
						<img src="${cPath }/resources/admission/assets/img/gallery/about2.png" alt="">
					</div>
					<div class="about-back-img ">
						<img src="${cPath }/resources/admission/assets/img/gallery/about1.png" alt="">
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<!-- About Law End-->